...
import { APP_CONFIG, getFullTitle, getCustomUrl } from "../config/appConfig";
 ...
 * Incluye answer y session_id opcional para mantener el contexto de la sesión
 */
...
export default function ChatSpecialized() {
  ...
  async function onSend(e: React.FormEvent) {
    ...
    try {
      ...
      // Construir el cuerpo de la petición incluyendo session_id si existe
      const requestBody: { question: string; session_id?: string } = {
        question: q
      };
      
      // Incluir session_id solo si existe
      if (sessionId) {
        requestBody.session_id = sessionId;
        console.log("[CUSTOM] Incluyendo session_id:", sessionId);
      }
      ...
      // Extraer y guardar session_id si viene en la respuesta
      if (data.session_id) {
        setSessionId(data.session_id);
        console.log("[CUSTOM] Session ID recibido y guardado:", data.session_id);
      }
          ...
          <div className="w-full max-w-6xl bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-white">
              ...
              <button
                onClick={() => {
                  clearHistory();
                  setSessionId(null); // Limpiar también el session_id al limpiar el historial
                }}
